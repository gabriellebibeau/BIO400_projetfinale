...
steps <- 300 #temps auquel l'équilibre aura surement été atteint
...
for(i in 0:100){
  for(j in 0:100){
    ...
    if(sd(LV5_sol[(0.75*steps):steps,'P']) > limsd & sd(LV5_sol[(0.75*steps):steps,'N']) > limsd){ #Instable
      ...
      #Code pour trouver les positions des endroits avec de l'instabilité, et intepréter graphiquement le résultat
...
lim <- 0.01 #Puisque le résultat de l'équation différentiel peut s'approcher de 0 sans ne jamais l'atteindre, les conditions P, N et R = 0 peuvent ne jamais être atteinte.
            # La mise en place d'une limite où on assume que la densité P, N ou R est égal à 0 permet de mieux modéliser les différents étapes d'équilibre.
limsd <- 0.0001
steps <- 300 #pas de temps que nous jugeons adéquat pour représenter les états du système
...
#Visualisation
image (figure_1B, col = c("cyan2", "salmon", "green", "black"), breaks = 0:4, xlab = 'b_prime' , ylab = 'a_prime')
legend(x = 'bottomleft', legend = c('Instable', 'Proie IG exclu','Coexistence stable', 'Prédateur IG exclu'), fill = 1:4)
...
Finalement, il est à noter que toutes les figures ont été produites à 300 pas de temps pour avoir la certitude que l'équilibre était atteint. Or, des instabilités exactement comme dans l'article de R.D. Holt et G.A. Polis (1997) étaient visibles à 100 pas de temps (les valeurs n'avaient pas encore convergé vers l'équilibre), et ce dans la figure 1A et la figure 1B. Cela pourrait expliquer en partie pourquoi nous obtenons des résultats différents de l'article. Nous concluons donc que l'article est difficilement reproductible puisqu'il ne définit pas bien comment reproduire l'état de stabilité et que cet état peut être trouvé à différent pas de temps sous différentes formes.
