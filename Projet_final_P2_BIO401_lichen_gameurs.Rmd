...
Dans le passé, plusieurs modèles ont été développés pour illustrer ces relations. Il y a le modèle consommateur-ressource, une généralisation de plusieurs modèles dont le modèle proie-prédateur de Lotka-Voltera. @holt_theoretical_1997 ont bonifié ce dernier de PIG (une ressource consommée par une proie IG et un prédateur IG). Ils ont trouvé que cet ajout menait à 4 états d'équilibres : une absence totale de proie intraguilde (IG), une absence totale de prédateur IG, un équilibre instable et une coexistence. Notre modèle précédent ne permettait pas d'atteindre d'instabilité, et ce peu importe les paramètres utilisés. Nous allons donc ne pas tenir en compte les états d'équilibre instable. Nos objectifs sont de déterminer les équilibres alternatifs qui existent dans un cas de PIG complexe comprenant une ressource et trois utilisateurs (proie IG, prédateur IG et prédateur suprême IG). En d'autres mots, nous avons introduit un prédateur suprême IG opportuniste (qui veut autant manger le prédateur IG que la proie IG) au modèle de @holt_theoretical_1997 basé sur Lotka-Voltera, puis fait varier le type de prédateur suprême IG pour voir si cela amenait des changements dans les états d'équilibre. Nous considérons que le prédateur suprême IG est un omnivore, malgré le nombre restreint d'espèces qu'il consomme. Il n'a pas de préférence entre le prédateur IG et la proie IG. Nous testerons quatre types de prédateurs suprêmes IG différents:

prédateur suprême IG de type 1: Le prédateur suprême IG est très bien adapté à la consommation du prédateur IG et de la proie IG. Il convertit donc efficacement ses derniers en reproduction pour lui-même. 

prédateur suprême IG de type 2: Le prédateur suprême IG est très bien adapté à la consommation du prédateur IG, et peu adapté à la consommation de la proie IG. Il convertit donc efficacement seulement le prédateur IG en reproduction pour lui-même.

prédateur suprême IG de type 3: Le prédateur suprême IG est très bien adapté à la consommation de la proie IG, et peu adapté à la consommation du prédateur IG. Il convertit donc efficacement seulement la proie IG en reproduction pour lui-même.

prédateur suprême IG de type 4: Le prédateur suprême IG n'est pas bien adapté à la consommation ni du prédateur IG, ni de la proie IG. Il convertit donc peu efficacement ces derniers en reproduction pour lui-même. Il est donc beaucoup plus dépendant de la ressource pour se nourrir.
...
$a''$ : la réponse fonctionnelle du prédateur suprême IG vers la ressource commune;
$b''$ : le taux de conversion de la consommation de la ressource vers la reproduction du prédateur suprême IG;
$m''$ : le taux de mortalité densité-dépendant du prédateur suprême IG;
$\psi$ : le taux de conversion de la consommation du prédateur IG vers la reproduction du prédateur suprême IG;
$\phi$ : le taux de conversion de la consommation de la proie IG vers la reproduction du prédateur suprême IG;
$\gamma$ : la réponse fonctionnelle du prédateur suprême IG vers le prédateur;
$\delta$ : la réponse fonctionnelle du prédateur suprême IG vers la proie IG.
...
Les figures montrant la dynamique de la population des 4 espèces ont été générées à l'aide du code suivant. Voici l'exemple pour visualiser l'instabilité du modèle à la figure 4.1.
...
On pourrait donc supposer que plus le prédateur suprême IG de type 1 est adapté à la ressource, plus il est compétitif et est capable d'exclure complètement les autres membres hétérotrophes de la communauté. Ceci pourrait être dû à sa grande capacité à profiter des bénéfices de s'alimenter de ces derniers.

#### Figure 4.2
La figure 4.2 ressemble beaucoup à la figure 4.1, mais la zone d'exclusion du prédateur suprême IG est beaucoup plus grande. Le prédateur IG n'est jamais le seul à être exclu: en fait, étrangement, il semble être exclu, pour une petite région de la figure, avec le prédateur suprême IG. Ceci pourrait être explicable au fait que le prédateur suprême IG dévore les prédateurs IG à un tel point que sa survie en est affectée: il est ensuite incapable de compétitionner avec la proie pour la ressource (Figure 7).
...
Il y a quelques zones d'instabilité, mais suite à une analyse plus approfondie, il s'agit d'une erreur causée par le choix du pas de temps. En effet, il s'agirait plutôt de zone où le prédateur suprême et la proie sont exclus, comme dans la figure 4.2. Cela n'arrive seulement pas assez rapidement. (Figure 9)
...
#### Figure 5.3
La figure 5.3 suit la même logique que la figure 4.3, mais avec un prédateur suprême IG de type 3 moins compétitif dans l'environnement différent que fait la deuxième suite de paramètres. Il se pourrait que le prédateur, bien adapté à consommer la proie, soit capable de tenir tête plus longtemps au prédateur suprême IG de type 3. 

#### Figure 5.3
La figure 5.4 suit la même logique que la figure 4.4, mais avec une zone de coexistence au lieu d'une zone d'exclusion du prédateur IG et de la proie IG. Ceci suggère que la réduction de la mortalité de la proie IG ainsi que l'augmentation de la capacité du prédateur IG à ce nourrir de la proie, maintenant plus abondante, soit un bon tampon à un prédateur suprême IG de type 4 très compétitif pour la ressource, mais peu adapté à ce nourrir de la proie et du prédateur IG. De plus, comparativement à la figure 1B du modèle proie-prédateur de Lotka-Voltera bonifié de PIG par R.D. Holt et G.A. Polis (1997), la proie IG n'est jamais exclue: il semblerait que, lorsque très compétitif pour la ressource, le prédateur suprême IG de type 4 soit capable de contrôler la population de prédateur IG, imposant ainsi une coexistence stable. Une touche d'instabilité lorsque le prédateur suprême IG est à son plus compétitif pour la ressource suggère que, dans cette situation, l'équilibre qu'il impose est plus fragile.


### Conclusion

En bref, l'ajout d'un prédateur suprême IG n'a pas augmenté la coexistence de toutes les espèces comparativement au modèle proie-prédateur de Lotka-Voltera bonifié de PIG, mais elle semble avoir favorisé la coexistence du prédateur IG et de la proie. En effet, dans certains cas, il semblerait que le prédateur suprême IG joue un rôle de régulateur de la population de prédateur IG. Cette hypothèse est basée entre autres sur le fait qu'aucun état où le prédateur IG exclue le prédateur suprême IG ainsi que la proie IG n'a été observé. Le système prédateur IG-proie IG est souvent tolérant d'un prédateur suprême IG peu compétitif pour la ressource. L'ajout d'un prédateur suprême IG a aussi apporté une plus grande diversité d'état d'équilibre ainsi que, dans de rares occasions, un réel état d'instabilité dans une communauté de proie IG, prédateur IG, ressource commune, et lui-même. Cela nous permet donc de confirmer les résultats de Andersson 1997, soit que l'ajout d'omnivores généralistes a tendance à stabiliser la coexistence dans une communauté.

Pour conclure, cette étude aurait bénéficié d'un appareil d'analyse plus puissant. En effet, d'augmenter la résolution des figures ainsi que les pas de temps aurait permis d'avoir une idée plus nette de la réalité des interactions entre les 4 espèces à long terme. Cependant, comme mentionné dans @polis_complex_1991, il est possible qu'en pratique, cela ne soit pas représentatif de la réalité. En effet, une population peut atteindre un seuil critique qui la mènera à l'extinction locale. Nos modèles montrent parfois, comme pour Polis (1991), des espèces dont la population descend à un seuil si bas qu'en nature elle sera éteinte, pour ensuite remonter à une population plusieurs dizaines d'ordres de grandeurs plus populeuses (l'équivalent,dans le cas de la figure 4.2, que si la population du prédateur suprême IG passait de 1 à 1X10^35, soit plus que le nombre total de cellules sur Terre). De plus, lorsque le nombre de pas de temps est très grand, les paramètres peuvent changer: les processus évolutifs influencent à long terme les populations de vivant, et notre modèle aurait besoin de beaucoup d'équations pour prédire cela.
## Références
