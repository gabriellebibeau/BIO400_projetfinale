...
Dans une communauté écologique, il y a plusieurs espèces fauniques, fongiques et floristiques. Il existe donc plusieurs différentes relations du type consommateur-resource entre elles Ces relations forment de grands réseaux reliant toutes les espèces entre elles que l'on nomme réseau trophique. Un exemple bien documenté de réseau trophique est celui de la vallée de Coachella en Californie (Polis 91). Ce réseau très complexe comporte  Une espèce peu faire partie de plusieurs niveaux trophiques en consommant différentes ressources (biotiques et abiotiques). Il est alors question de prédation intraguilde (PIG). En d'autres mots, il y a deux consommateurs d'une même ressource qui entre eux ont une relation proie-prédateurs.
...
Nous avons tout d'abord compilé la figure d'équilibres suivante grâce à notre modèle de prédation intraguilde complexe : 

![Figure 2. Les différents équilibres dans une communauté avec trois espèces sous l'effet de PIG modélisé avec Lotka-Voltera ($m'' = 0.2$, $\gamma = 0.4$, $\delta = 0.4$, $\phi = 0.8$ et $\psi = 0.8$).](FigS.png)

Les valeurs des paramètres utilisés dans notre modèle pour arriver à cette figure sont décrit dans la section méthode, et cela va pour toutes les autres figures présentées.

***

Ensuite, nous avons créé cette figure d'équilibres, toujours grâce à notre modèle de prédation intraguilde complexe : 

![Figure 3. Les différents équilibres dans une communauté avec trois espèces sous l'effet de PIG modélisé avec Lotka-Voltera ($m'' = 0.2$, $\gamma = 0.4$, $\delta = 0.4$, $\phi = 0.8$ et $\psi = 0.8$).](FigS.png)

***

Puis, nous avons illustré les équilibres de notre modèle dans la figure suivante : 

![Figure 4. Les différents équilibres dans une communauté avec trois espèces sous l'effet de PIG modélisé avec Lotka-Voltera ($m'' = 0.2$, $\gamma = 0.4$, $\delta = 0.4$, $\phi = 0.8$ et $\psi = 0.8$).](FigS.png)

***

Finalement, nous avons observé les équilibres créé par un quatrième type de prédateur dans notre modèle à travers cette figure : 

![Figure 5. Les différents équilibres dans une communauté avec trois espèces sous l'effet de PIG modélisé avec Lotka-Voltera ($m'' = 0.2$, $\gamma = 0.4$, $\delta = 0.4$, $\phi = 0.8$ et $\psi = 0.8$).](FigS.png)

## Discussion
Avant de calculer les équilibres de notre modèle, nous croyions que l'ajoût d'un prédateur suprême dans l'écosystème permettra des états alternatifs d'équilibres autant stables que instables.

Nous pensions aussi qu'il y aurait des exclusions (6) et une coexistance stable.

Finalement, nous avions estimé une augmentation des taux de réponse fonctionnelle vers la ressource commune et de conversion de la ressource commune en nouveaux individus.

Interprétation résultats (Type de prédateurs) (exemple nature, comparaison possible?)

En conclusion, 

Notre modèle ainsi que tout ceux le précédent sont entre autre bati pour des relations entre espèces appartenant au règne animal et/ou au règne végétal. Toutefois, une partie importante de nos écosystème grouille sous la terre : la fonge. Inclure à notre modèle de PIG un mycète capable de fournir une source de nourriture supplémentaire aux proies et aux deux prédateurs ainsi qu'augmenter le renouvellement de la ressource commune (en assumant que cette dernière est une plante) serait fort intéressant.
